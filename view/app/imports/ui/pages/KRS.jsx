  ...
  constructor(props) {
    ...
    monthlyBoxTrends.call({ box_id: 4, month: 0, year: 2018 }, (error, { dailyTrends }) => {
      if (error) {
        ...
        this.state.dailyTrends = _.values(dailyTrends);
        console.log(this.state.dailyTrends);
      }
    });
  }

  render() {
    const trendsReady = this.props.trendsReady;
    const trends = this.props.trends;
    if (trendsReady) {
      const voltages = trends.map((trend) => ({
            ...
            <Grid.Row>
              <Grid.Column textAlign={'center'}>
                ...
                <ResponsiveContainer width={'100%'} aspect={3}>
                  <AreaChart data={this.state.dailyTrends}>
                    <XAxis />
                    <YAxis />
                    <CartesianGrid />
                    <Legend />
...
export default withTracker(() => {
  ...
  return {
    trends,
    trendsReady: trendsHandle.ready(),
  };
})(KRS);
