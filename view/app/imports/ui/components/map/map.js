...
Template.map.onRendered(function mapOnRendered() {
  ...
  L.Icon.Default.imagePath = '/packages/bevanhunt_leaflet/images/'; // eslint-disable-line no-undef
  template.mainMap = L.map('main-map').setView([21.466700, -157.983300], 10); // eslint-disable-line no-undef
  template.markerLayerGroup = L.layerGroup([]); // eslint-disable-line no-undef

  const osmUrl = 'http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';
  const osmAttrib = 'Map data © OpenStreetMap contributors';
  const osm = new L.TileLayer(osmUrl, { attribution: osmAttrib }); // eslint-disable-line no-undef
