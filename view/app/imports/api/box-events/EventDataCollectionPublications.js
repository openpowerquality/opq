...
export const eventDataCollectionPublications = () => {
  Meteor.publish(BoxEvents.publicationNames.EVENT_DATA, function(requestId) {
    check(requestId, Number);
    console.log('sub attempt');
    return BoxEvents.find({request_id: requestId}, {});
  });

  // Meteor.publish(BoxEvents.publicationNames.RECENT_EVENT_DATA, function() {
  ...
  //   const requestIds = BoxEvents.find({}, {sort: {_id: -1}})
  //       .fetch()
