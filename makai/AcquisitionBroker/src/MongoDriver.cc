...
uint32_t MongoDriver::get_next_event_number() {
    ...
    opts.limit( 1 );
    opts.sort(document{} << "event_number" << -1 << finalize);
    auto result = _event_collection.find_one(document{}.view(), opts);
    if (!result) return 1;
    int32_t next_event_number = result->view()["event_number"].get_int32() + 1;
...
bool MongoDriver::create_event(opq::proto::RequestEventMessage &m, uint64_t ts, uint32_t event_num) {
    ...
    builder << "type" << opq::proto::RequestEventMessage_TriggerType_Name(m.trigger_type())
            << "event_number" << (int32_t)event_num
            << "description" << m.description();

    auto box_array = builder << "boxes_triggered" << open_array;
    std::for_each(m.box_ids().begin(), m.box_ids().end(),
                  [&box_array](int box){box_array << box;});

    box_array << close_array;
    builder << "boxes_recieved" << open_array << close_array;
    builder << "event_start" << (int64_t)m.start_timestamp_ms_utc()
            << "event_end" << (int64_t)m.end_timestamp_ms_utc();
...
bool MongoDriver::append_data_to_event(std::vector<opq::proto::DataMessage> &messages, uint32_t event_num) {
    ...
    _event_collection.update_one(document{}
                                 ...
                                 document{}
                                         ...
                                         << open_document
                                               <<"boxes_recieved" << id
                                               <<"time_stamp" << (int64_t)chrono_to_mili_now()
                                         << close_document
                                         << finalize);
    ...
    auto data_array_context = builder << "data" << bsoncxx::builder::stream::open_array;
    for (auto &message : messages){
        for(auto &cycle : message.cycles()) {
