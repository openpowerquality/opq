...
pub fn app_to_box(ctx: Arc<zmq::Context>, settings: Settings) {
    ...
    pub_sock
        .set_curve_secretkey(&settings.sec_key.unwrap())
        .unwrap();
    pub_sock
        .set_curve_publickey(&settings.pub_key.unwrap())
        .unwrap();
    ...
    loop {
        ...
        lock.id_map
            .insert(sequence, (message.identity.clone(), Instant::now()));
        println!(
            "New command from identity {} to box {}, sequence {}.",
            message.identity, message.box_id, message.seq
        );
