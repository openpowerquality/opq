...
if __name__ == "__main__":
    ...
    total_events = box_events.count({"thd": {"$exists": False},
                                     "itic": {"$exists": False}})
    print("Total box_events", total_events)
    i = 0
    pool = multiprocessing.Pool(initializer=init_parallel_client)
    for v in pool.imap_unordered(parallel_thd_itic_fn, box_events.find({"thd": {"$exists": False},
                                                                        "itic": {"$exists": False}}, ["_id", "data_fs_filename", "box_id"])):
        if i % 50 == 0:
